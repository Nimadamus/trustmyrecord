...
(function () {
  ...
  function injectHeaderCSS() {
    ...
    if (!nav) {
      ...
      if (!scale) {
        ...
        nav.rel = 'stylesheet';
        nav.href = '/static/css/tmr-navbar.css';
      }
    }
    if (nav) document.head.appendChild(nav);

    var HREF = '/static/css/tmr-ds-header.cd37174a63bb.css';
  ...
  var NOTIF_CHAIN = [
    ...
    '/static/js/backend-api.js?v=91fb62a52484',
  ...
  function init() {
    ...
    var ds = cl.contains('tmr-ds') || cl.contains('tmr-ds-shell');
    /* NAV-ONLY ADOPTION (2026-08-31). 129 pages loaded this script and then
       failed the gate above, so they rendered with no site header at all:
       /login/, /extra-markets/, every admin screen and every forum category
       page carry only `tmr-theme-a`, and the classic forum carries
       `classic-forum`. tmr-navbar.css already names both of those shells in
       every one of its selectors -- the stylesheet half of their adoption was
       written and the JS half was never opened. They take the BAR only: the
       forum has its own footer and its own section strip underneath, and a
       page with no footer at all would otherwise get an unstyled .ds-footer,
       because tmr-navbar.css carries no footer rules. */
    var navOnly = !ds && (cl.contains('tmr-theme-a') || cl.contains('classic-forum'));
    if (!ds && !navOnly) return;
    injectHeaderCSS();
    buildNav();
    if (ds) buildFooter();
