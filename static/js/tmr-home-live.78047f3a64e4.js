...
(function () {
  ...
  function loadGameFile() {
    return fetch(API + '/matchups/today', { credentials: 'omit' })
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (d) {
        var f = d && d.ok && d.featured;
        if (!f || !f.url) return;
        // Only a LIVE article is worth linking to.
        if (f.status !== 'published' && f.status !== 'updated') return;
        gameFile = {
          url: f.url,
  ...
  function applyGameFile() {
    ...
    /* On production the ticker is server-rendered by the tmr-home-ssr Worker,
       ...
       are simply already in the document. Matching used to depend on that
       payload, which is why this worked locally and did nothing once deployed.
       When the slate is not in hand, fetch it: it is the same cached endpoint
    ...
    if (!card || card.classList.contains('gm--gf')) return;

    card.classList.add('gm--gf');
    card.setAttribute('href', gameFile.url);
    ...
    var label = teams.length === 2
      ...
      : 'this matchup';
    card.setAttribute('aria-label', "Today's TMR Game File: " + label + '. Read the full matchup analysis.');
