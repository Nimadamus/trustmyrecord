(() => {
    ...
    // ONE small section, consolidated 2026-08-28. See the long note in
    // tmr-ds-nav.js: the hub, the directory, the Arena and the open board were
    // four rows over one area, so /online-gaming/ became the single destination
    // carrying the games and the live challenge board together. Rankings and
    // leagues are different screens and stay. Nothing was retired; the Arena,
    // the hub and the game pages are linked from the page body.
    ...
    const routeMeta = {
        ...
        "online-gaming.html": ["Online Gaming", "Play MLB The Show, Madden NFL, NBA 2K, EA SPORTS FC and EA SPORTS NHL head-to-head. The games, the live open-challenge board and the create form on one page."],
        ...
        return `
            <span class="tmr-user-chip-wrap">
                ...
                <a href="/wallet/" id="navCoinChip" class="tmr-global-nav__coins" data-tmr-coin-balance aria-label="${(window.TMR_TERMINOLOGY && window.TMR_TERMINOLOGY.full) || 'TMR Coin'} balance" title="${(window.TMR_TERMINOLOGY && window.TMR_TERMINOLOGY.full) || 'TMR Coin'} balance" hidden>
                    <img class="tmr-global-nav__coins-icon" src="/static/branding/tmr-coin/tmr-coin-logo.svg" alt="" aria-hidden="true"><span id="navCoinBalance">0</span>
                </a>
        ...
        <div class="tmr-global-nav__inner">
            ...
            <div class="tmr-global-nav__panel">
                ...
                <div class="tmr-support-menu tmr-sports-gaming-menu${SPORTS_GAMING_GROUP.has(currentFile) ? " is-current" : ""}">
                    ...
                    <div class="tmr-support-menu__panel" role="menu" aria-label="Online Gaming links">
    ...
    async function runSearch(q) {
        ...
        if (threads.length) {
            sections.push(
                ...
                threads.map(t => `<a class="tmr-search-result" href="${t.slug ? `/forum/thread/${t.id}/${encodeURIComponent(t.slug)}/` : `/forum/?thread=${t.id}`}"><strong>${escapeHtml(t.title || "Thread")}</strong><span>${escapeHtml(t.category_name || t.username || "")}</span></a>`).join("")
            );
        }
    ...
    if (!document.querySelector('script[data-tmr-nav-badges]')) {
        ...
        navBadgesScript.src = "/static/js/nav-badges.js?v=dca92f04da03";
...
(function () {
    ...
    s.src = '/static/js/tmr-featured.js?v=6d1832cc9f86';
    s.defer = true;
