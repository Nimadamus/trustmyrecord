...
(function () {
  ...
  var SPORTSBOOK = [
    ...
    ['@sub', 'Featured Matchups', [
      /* FMENU rows are rewritten by scripts/featured_matchups.py on every
      ...
      /*FMENU tennis*/ ['/matchup-of-the-day/joint-tararudee-the-quarterfinals/', 'Tennis', '/matchup-of-the-day/tennis/'],
    ...
    /* HANDICAPPING HUB, 2026-09-08. 'MLB Matchups Today' used to sit here as a
       ...
       All seven leagues, since 2026-09-08. NCAAF and Soccer shipped five rows
       short because /handicapping/ncaaf/ and /handicapping/soccer/ did not
       exist; Nima's answer was that they all belong in the hub, so both hubs
       were built and both rows went in with them. Those two are board-only
       pages: the graded game database behind the matchup pages carries MLB,
       NBA, NFL and NHL and nothing else, so they list the day's fixtures and
  ...
  var TOOLS = [
    ...
    /* Model Builder rejoined this menu on 2026-09-08. /model-builder/ has been
       ...
       page. It is one of the analytical products, so it belongs in the shared
       TOOLS list every page renders from. Nothing else moved. */
  ...
  /* CACHE NOTE 2026-09-08: the previous build of this file was requested at the
     edge before its deploy landed, so Cloudflare cached the 404 for four hours
     and every repointed page loaded no nav at all. Republished under a new hash
     the edge has never seen. Never GET a hashed asset URL before its deploy is
    ...
    return '<div class="ds-menu' + (on ? ' is-current' : '') + '" data-menu="' + esc(key || label.toLowerCase()) + '">' +
      ...
      list.map(function (r) {
        ...
        return '<div class="ds-sub' + (subOn ? ' is-current' : '') + '">' +
          ...
          kids.map(function (k) {
            var door = (typeof k[2] === 'string') ? ' data-tmr-door="' + k[2] + '"' : '';
            return '<a href="' + k[0] + '"' + door + ' role="menuitem"' + (k[0] === cur ? ' aria-current="page"' : '') + '>' + esc(k[1]) + '</a>';
          }).join('') + '</div></div>';
      }).join('') + '</div></div>';
  }
  ...
  var NOTIF_CHAIN = [
    ...
    '/static/js/backend-api.js?v=10c8f87858c0',
  ...
  loadChain(['/static/js/tmr-ds-avatar.a6e8c49d08e7.js']);

  /* FEATURED MATCHUPS DIRECT (2026-09-15). Every Featured Matchups menu entry
     (MLB, NCAAF, NFL, tennis, soccer, desktop and mobile) opens today's article
     itself, never the /matchup-of-the-day/<sport>/ door. tmr-featured.js reads
