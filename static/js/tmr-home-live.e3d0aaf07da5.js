...
(function () {
  ...
  function applyGameFile() {
    ...
    for (var pk in byPk) {
      ...
      if (decorate(lane, pk, byPk[pk].url)) marked++;
    }
    ...
    if (haveMap && lastSlate && lastSlate.games) {
      for (var pk2 in byPk) {
        ...
        for (var i = 0; i < lastSlate.games.length; i++) {
          ...
          if (decorate(lane, String(g.game_pk == null ? '' : g.game_pk), art.url)) marked++;
        }
      }
    }
  ...
  function decorate(lane, pk, url) {
    ...
    var label = teams.length === 2
      ...
      : 'this matchup';
    card.setAttribute('aria-label', 'TMR breakdown: ' + label + '. Read the full matchup analysis.');

    /* Absolutely positioned so the strip's height and every other card stay
       exactly where they were. */
    var badge = document.createElement('span');
    badge.className = 'gm-gf';
    /* "VIEW BREAKDOWN", not "TMR GAME FILE". The old label named the product;
       this one tells the reader what happens when they click, which is the only
       job a badge on a ticker card has. */
    badge.textContent = 'VIEW BREAKDOWN';
  ...
  /* ---------------------------------------------------------- the call-out
     A slim bar directly under the strip naming the game and inviting the click.

     The badge on a ticker card is small, it sits on a strip that pages every
     few seconds, and the flagged game is often not on the first page — so on a
     cold load a reader can easily never see it at all. This is the second,
     stationary entry point.

     INJECTED, never baked into index.html. The homepage markup is locked
     (tests/homepage-approved-baseline-lock-test.js) and a full-viewport matchup
     cover was built here once and rejected outright; adding static markup is
     how that comes back by accident. This element exists only when the API says
     there is an article to point at, and it removes itself when there is not.

     It is inserted AFTER the ticker section rather than inside it, so nothing
     about the strip's own height or card geometry changes. */
  function renderCallout() {
    var existing = el('#tmr-gf-callout');

    var first = null;
    for (var k in gameFiles) {
      if (Object.prototype.hasOwnProperty.call(gameFiles, k)) { first = gameFiles[k]; break; }
    }
    if (!first || !first.url) { if (existing) existing.parentNode.removeChild(existing); return; }

    var strip = el('.ticker');
    if (!strip || !strip.parentNode) return;

    var matchup = (first.away_team || '') + ' at ' + (first.home_team || '');
    var line = first.angle_label || first.title || 'The full matchup breakdown';

    var box = existing || document.createElement('a');
    box.id = 'tmr-gf-callout';
    box.className = 'gf-callout';
    box.setAttribute('href', first.url);
    box.innerHTML =
      '<span class="gf-callout-k">Today&rsquo;s breakdown</span>' +
      '<span class="gf-callout-m">' + esc(matchup) + '</span>' +
      '<span class="gf-callout-d">' + esc(line) + '</span>' +
      '<span class="gf-callout-cta">Read the breakdown &rarr;</span>';

    if (!existing) strip.parentNode.insertBefore(box, strip.nextSibling);
  }
