...
(function () {
  ...
  function injectCSS() {
    ...
    /* PROFILE_UI_20260917. Everything this fallback renders is styled by the same
       tokens the baked page ships in its <style> block, so a visitor who lands on
       the fallback gets one coherent TMR profile rather than the baked header
       followed by a differently-styled set of generic tables. Rules here only
       cover the classes this script INVENTS (.u-substats, .u-num, .u-bw, the
       filter, the pending badge, the alignment classes it puts on its own
       tables); the shared .u-stat/.u-table look lives in the baked stylesheet. */
    var css =
      '.u-substats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:0;}' +
      '.u-substats .u-stat b{font-size:clamp(17px,1.7vw,21px);}' +
      '.u-num.pos{color:#22e08a;}.u-num.neg{color:#ff5f70;}.u-num.zero{color:#8e98b4;}' +
      '.u-table td.pos,.u-table td .pos{color:#22e08a;font-weight:800;}' +
      '.u-table td.neg,.u-table td .neg{color:#ff5f70;font-weight:800;}' +
      '.u-table td.zero{color:#8e98b4;}' +
      '.u-t--split2 th:not(:first-child),.u-t--split2 td:not(:first-child){text-align:right;}' +
      '.u-t--picks2 th:nth-last-child(-n+2),.u-t--picks2 td:nth-last-child(-n+2){text-align:right;}' +
      '.u-filter{display:inline-flex;gap:8px;align-items:center;margin:0 0 10px;font-size:12.5px;color:#8e98b4;}' +
      '.u-filter select{background:#0e0e16;color:#eef1f8;border:1px solid #23233a;border-radius:9px;padding:6px 10px;font:inherit;}' +
      '.u-badge{display:inline-block;font-size:10px;font-weight:800;letter-spacing:.09em;padding:3px 9px;border-radius:999px;text-transform:uppercase;}' +
      '.u-badge.pend{color:#ffd166;border:1px solid rgba(255,209,102,.4);background:rgba(255,209,102,.08);}' +
      '.u-bw{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px;}' +
      '.u-bw .u-stat{flex-direction:column;}' +
      '.u-bw .u-stat b{font-size:clamp(17px,1.7vw,21px);}' +
      '.u-bw .u-stat span{display:block;font-size:12px;letter-spacing:.02em;text-transform:none;color:#b9c2d8;font-weight:700;}' +
      '.u-bw .u-bwh{color:#7f89a6;font-size:10.5px;text-transform:uppercase;letter-spacing:.1em;font-weight:800;margin:0 0 5px;}' +
      '.u-scroll{overflow-x:auto;-webkit-overflow-scrolling:touch;}' +
      '@media(max-width:860px){.u-substats{grid-template-columns:repeat(2,minmax(0,1fr));}.u-bw{grid-template-columns:1fr;}}';
    var st = document.createElement('style');
  ...
  function statCard(big, label, klass) {
    /* The tone class is presentation only -- it is read off the sign of a number
       this script does not touch. It goes on the tile as well as the value so the
       tile's accent rail colours without relying on :has(). */
    var tone = (klass === 'pos' || klass === 'neg') ? ' is-' + klass : '';
    return '<div class="u-stat' + tone + '"><b' + (klass ? ' class="u-num ' + klass + '"' : '') + '>' + esc(big) +
  ...
  function renderHeadline(s, streaks) {
    ...
    var cards = [
      ...
      statCard(streakTxt(best), 'Best Streak', best > 0 ? 'pos' : ''),
      statCard(s.avg_odds != null ? amer(s.avg_odds) : '--', 'Avg Odds')
    ];
    box.innerHTML = cards.join('');
  }

  function tableBlock(title, head, bodyRows, note, extraTopHTML, tableClass) {
    return '<section class="u-block">' +
      '<h2>' + esc(title) + '</h2>' +
      (extraTopHTML || '') +
      '<div class="u-scroll"><table class="u-table' + (tableClass ? ' ' + tableClass : '') + '"><thead><tr>' +
  ...
  function renderDeep(metrics, picks) {
    ...
    if (graded.length) {
      html += tableBlock('Recent graded picks',
        ...
        'Most recent settled picks.', null, 'u-t--picks2');
    }
    ...
    if (graded.length) {
      ...
      html += tableBlock('Full pick history (' + graded.length + ')',
        ...
        filterHTML, 'u-t--picks2')
        .replace('<table class="u-table u-t--picks2">', '<table class="u-table u-t--picks2" id="uHistTable">');
    }
