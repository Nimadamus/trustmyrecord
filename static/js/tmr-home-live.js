...
(function () {
  ...
  var NEUTRAL_BODY = '<circle cx="50" cy="50" r="50" fill="#111827"/>'
  ...
  /* 40s as of 2026-09-08. Nima: "The score/game cards disappear too quickly
     right now. Keep each game/highlight visible noticeably longer so a user has
     enough time to actually read the score and the accompanying context without
     staring at the ticker waiting for it to come back around. Do not make it
     painfully slow, but increase the dwell time significantly."

     The number is NOT free-floating: a paged row caps each highlight at half
     the page window (see pagedDwellCap), so the page window is what actually
     decides how long a SENTENCE is readable. At 24s that cap was 11.5s, which
     is below the bottom of the postgame dwell band the lines were written for.
  ...
  /* 12s from 8s on 2026-09-08, same brief. A pregame line is one short clause
     and does not need a recap's dwell, but at 8s it was turning over while the
     reader was still on the score above it. */
  /* 5 SECONDS AT MOST (Nima, 2026-09-14: "when it shows a ticker highlight it
     should be up like no more than 5 seconds and then switch to another one").
     Pregame lines hold 5s; finals and live cards hold 4, 4.5 or 5s off their
     game id, so two cards side by side still do not flip together. */
  ...
  /* 20 to 28s from 14 to 22 on 2026-09-08, with the page window raised to match
     so the cap does not simply eat the increase. The five distinct values are
     what keep two finals side by side from flipping together. */
  var POSTGAME_DWELL_MIN_MS = 4000;
  var POSTGAME_DWELL_STEP_MS = 500;
  var POSTGAME_DWELL_STEPS = 3;
  ...
  var tkPaused = false;
  var tkWired = false;
  /* PRIORITY ORDER IS KEPT AS NODES. tkCanon is every card in the order
     tickerOrder() gave them; the carousel physically moves pages so the next
     one it visits is always the neighbour (see bringNext), and layoutTicker
     puts the cards back in this order before it re-pages. tkPagesCanon holds
     the pages as layoutTicker built them, tkVisit is the visiting sequence as
     indexes into it, tkVisitPos where the carousel is in that sequence. */
  var tkCanon = null;
  var tkAnchorKey = null;
  var tkPagesCanon = [];
  var tkVisit = [];
  var tkVisitPos = 0;
  var tkSportCount = 1;
  ...
  /* `noScore` is the scorebug's flag. On a one-line card the chip is the only
     place the score can go, so it carries it; on a scorebug each club already
     owns its own number and repeating them as "Final 3-66" is precisely the
     presentation Nima rejected on 2026-09-05. Defaulted off, so every existing
     caller - MLB, NFL, NBA, NHL - is byte-for-byte unchanged. */
  function statusChip(g, noScore) {
    ...
    var score = (!noScore && typeof g.away_score === 'number' && typeof g.home_score === 'number')
  ...
  /* ------------------------------------------------- THE SCOREBUG CARD
     ...
     Nima, 2026-09-05, looking at the college row on the live homepage:

       "New Hampshire   Syracuse   FINAL 3-66 ... the final score is detached
        from the teams and pushed awkwardly to the far right. It looks like raw
        database output rather than a professional sports scoreboard."

     So a college card is no longer a line of names with a chip on the end. It
     is a scorebug: the league and the game state across the top, then ONE ROW
     PER CLUB carrying that club's logo, its name and ITS OWN score in a fixed
     right-hand column, then the rotating strip underneath. The eye reads it the
     way it reads ESPN, CBS or Fox - who played, who won, what the score was, in
     one glance - because the number sits on the same line as the team it
     belongs to and the numbers line up with each other.

     IT IS NOT A COLLEGE COMPONENT. Every decision below is driven by the row
     key, so adding 'nfl' to SCOREBUG_SPORTS is the whole of moving professional
     football onto it. That is deliberately not done today: "The immediate
     assignment is CFB."

     NFL JOINED ON 2026-09-14 (Nima: a live game should read "DEN 10 / KC 31,
     Q4 14:39" with logos, the quarter, the clock and who has the ball). Same
     rule as college: a game with a score is a scorebug, an upcoming one keeps
     the one-line card.

     The client and workers/home-ssr/worker.mjs bake the SAME markup. They are
     in lockstep by hand; a change here is a change there or the first client
     render flickers. */
  var SCOREBUG_SPORTS = { cfb: 1, nfl: 1 };

  var SPORT_LABEL = { cfb: 'CFB', nfl: 'NFL', nba: 'NBA', nhl: 'NHL' };

  /* One club: logo, name, score. The score cell is rendered even when there is
     no score to put in it, so an upcoming game, a game in progress and a final
     are the same shape and nothing shifts sideways as a game starts. */
  function bugRow(logo, rank, name, abbr, score, win, ball) {
    var full = rankedName(rank, name);
    var brief = abbr ? rankedName(rank, abbr) : full;
    var inner = brief === full ? esc(full)
      : '<span class="sf">' + esc(full) + '</span><span class="sa">' + esc(brief) + '</span>';
    /* WHO HAS THE BALL. Only when the API names a side, which it does only for
       a live football game with a play in progress; otherwise nothing is
       drawn rather than a guess. Inside the name cell so the three column
       grid and the score column never move. */
  ...
  function scorebugCard(g, key) {
    var done = g.status === 'final';
    var aw = typeof g.away_score === 'number' ? g.away_score : null;
    var hm = typeof g.home_score === 'number' ? g.home_score : null;
    var scored = aw !== null && hm !== null;
    /* EMPHASIS ONLY ON A RESULT. A club leading at half is not a winner and a
       card that styles it as one is telling the reader something untrue. */
    var awWin = done && scored && aw > hm;
    var hmWin = done && scored && hm > aw;
    /* Last night's results say which night they were, so nobody can take one
       for something happening now. See the carryover note in
       services/mlbNavSlateService.js on the API. */
    var label = (SPORT_LABEL[key] || String(key).toUpperCase())
  ...
  function insightStrip(g) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      var meta = ins.sample
        ...
        : (ins.period || '');
      /* A BREAKING MOMENT carries the time it happened. The markup is the same
         whatever the time is, so the edge and the client draw identical bytes;
         refreshBreaking() decides on the reader's own clock whether it is still
         breaking and fills in how long ago. */
      var brk = ins.breaking && ins.event_at ? String(ins.event_at) : '';
  ...
  /* The ESPN rows (Nima, 2026-08-14 for the NFL row). No data-game-pk: the MLB
     preview treatment can never attach to them.

     AN UPCOMING GAME IS NOT A SCOREBUG (Nima, 2026-09-14: the Syracuse at Pitt
     card "looks totally different ... doesn't look like the others"). With no
     score the scorebug is two empty score columns under a header, beside
     one-line MLB and NFL cards. A game that has not kicked off draws the same
     one-line card as every other upcoming game; the scorebug is for a game
     with a score to show. */
  function espnCardHtml(g, key) {
    if (SCOREBUG_SPORTS[key] && g.status !== 'scheduled') return scorebugCard(g, key);
    return '<a class="gm gm--' + key + '" data-sport="' + key + '"' +
      ' href="' + esc(g.href || '/sportsbook/') + '">' +
      '<span class="gm-top">' +
  ...
  /* ------------------------------------------------ PRIORITY, ACROSS SPORTS

     Every card, whatever its sport, is one normalized event: a sport key and a
     game carrying status, scores, period or inning, clock and insights. The
     order is decided on that alone:

       tier 0  LIVE
       tier 1  delayed or suspended (a game still in progress, stopped)
       tier 2  FINAL today
       tier 3  upcoming today, in the feed's own start time order
       tier 4  anything off today's slate the API flagged on purpose
               (a lookahead fixture, a carryover result) and PPD / canceled

     Inside a tier the sports take turns, one card each, so ten baseball games
     cannot stack in front of one football game. The sport with FEWER cards in
     the tier goes first, then the fixed order below. Within a sport the feed's
     order is kept. Pure function: the edge worker carries the same port and
     the parity test holds the two together. */
  var TICKER_SPORT_ORDER = ['nfl', 'mlb', 'nba', 'nhl', 'cfb'];

  function tickerTier(g) {
    var s = String((g && g.status) || 'scheduled');
  ...
  function renderTicker(payload) {
    ...
    /* ONE STRIP, EVERY SPORT, IN PRIORITY ORDER (Nima, 2026-09-14: a live
       Broncos at Chiefs game sat behind eight MLB cards, including five
       finals, on page nine of ten - five minutes and twenty seconds after the
       page loaded, and then gone for six more). Rows used to be concatenated
       sport by sport, MLB first, so any sport after baseball was buried by
       baseball's volume. tickerOrder() is the one place order is decided. */
    var html = '';   // the TODAY label is its own static column in index.html
    tickerOrder([{ key: 'mlb', games: games }].concat(espnRows)).forEach(function (e) {
      html += e.key === 'mlb' ? mlbCardHtml(e.g) : espnCardHtml(e.g, e.key);
    });
    ...
    /* The game on screen, by identity: the rebuild below replaces every node,
       and the carousel may have moved pages around (see bringNext). */
    /* Only a carousel this script has laid out has a game "on screen": the
       first render replaces a skeleton or an edge bake in some other order,
       and anchoring to its first card would open the strip on that card
       instead of the top of the priority order. */
    var shownTrack = tkPagesCanon.length ? lane.querySelector('.ticker-track') : null;
    var shownPage = shownTrack && shownTrack.children[tkPageIndex];
    var shownCard = shownPage && shownPage.querySelector ? shownPage.querySelector('.gm') : null;
    if (shownCard) tkAnchorKey = cardKey(shownCard);

    lane.innerHTML = '<div class="ticker-track"><div class="ticker-page">' + html + '</div></div>';
    tkCanon = Array.prototype.slice.call(lane.querySelectorAll('.gm'));
    lane.setAttribute('data-slate-date', payload.slate_date || '');
    lane.setAttribute('data-render-sig', sig);
    lane.setAttribute('aria-busy', 'false');
    restoreRotation(lane, carried);
    refreshBreaking(lane);
    surfaceBreaking(lane);
  ...
  function layoutTicker() {
    ...
    /* The game on screen before this re-page, so the carousel stays on it. */
    var nowPage = track.children[tkPageIndex];
    var nowCard = nowPage && nowPage.querySelector ? nowPage.querySelector('.gm') : null;
    var anchorKey = tkAnchorKey || (nowCard ? cardKey(nowCard) : null);
    tkAnchorKey = null;
    /* Back into priority order: the carousel may have moved pages. */
    if (tkCanon) {
      var at = function (c) { var i = tkCanon.indexOf(c); return i < 0 ? tkCanon.length : i; };
      cards = cards.map(function (c, i) { return { c: c, i: i }; })
        .sort(function (x, y) { return (at(x.c) - at(y.c)) || (x.i - y.i); })
    ...
    var wasVisit = tkVisit.slice();
    tkPageCount = pages.length;
    var found = -1;
    if (anchorKey) {
      for (var pi = 0; pi < pages.length && found < 0; pi++) {
        for (var ci = 0; ci < pages[pi].length; ci++) {
          if (cardKey(pages[pi][ci]) === anchorKey) { found = pi; break; }
        }
      }
    }
    if (found >= 0) tkPageIndex = found;
    else if (tkPageIndex >= tkPageCount) tkPageIndex = 0;
    tkPagesCanon = Array.prototype.slice.call(track.children);
    tkVisit = visitSequence(pages);
    tkSportCount = pages.reduce(function (acc, grp) {
      var sp = grp[0] ? (grp[0].getAttribute('data-sport') || 'mlb') : 'mlb';
      if (acc.indexOf(sp) < 0) acc.push(sp);
      return acc;
    }, []).length;
    /* THE SEQUENCE POSITION SURVIVES A REBUILD. A live slate re-pages on most
       90s refreshes; restarting the sequence each time would replay its first
       few pages forever. Same sequence: keep the position. Otherwise find the
       page on screen from where the old position was. */
    var same = wasVisit.length === tkVisit.length && wasVisit.every(function (v, i) { return v === tkVisit[i]; });
    if (!same || tkVisit[tkVisitPos] !== tkPageIndex) tkVisitPos = visitFind(tkPageIndex, same ? tkVisitPos : 0);
  ...
  function applyTickerPage(fromIndex, keepDwell) {
    ...
    markPaged(track.children[tkPageIndex]);
    ...
    if (lbl && first) {
      /* A strip carrying more than one sport names every page's league,
         baseball included, so nobody has to work out which sport a card is.
         A baseball-only strip keeps "Today". */
      var sp = first.getAttribute('data-sport') || 'mlb';
      var want = sp === 'mlb' ? (tkSportCount > 1 ? 'MLB' : 'Today') : (SPORT_LABEL[sp] || 'Today');
  ...
  /* ------------------------------------------------ THE VISITING SEQUENCE
     ...
     The strip shows one page at a time, and a page never mixes sports. Played
     front to back, a night of ten baseball pages and one football page shows
     the football game for 40 seconds in every seven minutes. So the carousel
     does not walk the pages in a straight line: the sports take turns, one
     page each, and a sport that has run out of new pages comes back round
     ONLY through its LIVE pages. A live game in the smaller sport is on screen
     every other turn; a final or an upcoming fixture is still visited exactly
  ...
  /* Position in the sequence of page `idx` (an index into tkPagesCanon),
     searching forward from `from` so a page that recurs keeps its place. */
  function visitFind(idx, from) {
    var n = tkVisit.length;
    for (var k = 0; k < n; k++) {
      var p = ((from || 0) + k) % n;
      if (tkVisit[p] === idx) return p;
    }
    return 0;
  }

  /* THE NEXT PAGE IS ALWAYS THE NEIGHBOUR. The track slides by whole pages,
     so a visit to a page six places back would sweep the reader across six
     pages in under a second. Instead the page to visit is moved to sit right
     after the one on screen, and the slide is the same one page move it has
     always been. If the move shifts the page on screen one place left, the
     track is re-positioned under it with the transition off, so nothing
     visibly moves. Returns the index of the page on screen after the move. */
  function bringNext(track, target) {
    var cur = track.children[tkPageIndex];
    if (!cur || !target || target === cur || target.parentNode !== track) return -1;
    if (cur.nextSibling !== target) {
      track.insertBefore(target, cur.nextSibling);
      var now = Array.prototype.indexOf.call(track.children, cur);
      if (now !== tkPageIndex) {
        var was = track.style.transition;
        track.style.transition = 'none';
        track.style.transform = 'translateX(-' + (now * 100) + '%)';
  ...
  function advanceTicker() {
    ...
    /* A PAGE WITH A MOMENT NOBODY HAS SEEN GOES NEXT, ahead of the sequence,
       which then carries on from where it was. */
    var hot = track ? breakingPage(track) : null;
    if (hot) {
      var fromHot = bringNext(track, hot);
      if (fromHot >= 0) {
        tkPageIndex = fromHot + 1;
        surfaceBreaking(hot, true);
        applyTickerPage(fromHot);
        return;
      }
    }
    /* A one-sport strip walks the pages in a straight line exactly as it always
       has, wrap included. */
    if (track && tkSportCount > 1 && tkVisit.length > 1 && tkPagesCanon.length === tkPageCount) {
      var pos = (tkVisitPos + 1) % tkVisit.length;
      var from = bringNext(track, tkPagesCanon[tkVisit[pos]]);
      if (from >= 0) {
        tkVisitPos = pos;
        tkPageIndex = from + 1;
        applyTickerPage(from);
        return;
      }
    }
  ...
  function goTicker(delta) {
    ...
    applyTickerPage(leaving);
    /* The arrows walk the pages as they sit; the sequence picks up from the
       page the reader chose. */
    var lane = el('.ticker .ticker-games');
    var track = lane && lane.querySelector('.ticker-track');
    var idx = track ? tkPagesCanon.indexOf(track.children[tkPageIndex]) : -1;
    if (idx >= 0) tkVisitPos = visitFind(idx, tkVisitPos);
    updateTickerNav();
  ...
  function cardKey(card) {
    ...
    if (pk) return 'pk:' + pk;
    var teams = [];
    /* Both card shapes: the one-line card's team spans and the scorebug's team
       names. Without the second selector every college card keyed as 'cfb:' and
       the rotation restore matched the wrong game to the wrong sentence. */
    card.querySelectorAll('.gm-top .t, .gb-r .gb-tn').forEach(function (t) { teams.push(t.textContent.trim()); });
    return (card.getAttribute('data-sport') || 'mlb') + ':' + teams.join('@');
  }

  /* The sentence each card is showing right now, keyed by game. */
  /* ------------------------------------------------ LAST KNOWN GOOD SLATE

     Nima, 2026-09-15, after a backend deploy at 11:38 PM PDT: the API answered
     twice with an HTML page and once with an MLB row of zero games, and a
     homepage that happened to refresh inside that minute showed "Today's games
     are temporarily unavailable" or dropped baseball altogether. A restart, a
     malformed answer, a timeout or a momentarily empty row must not take down
     a strip that was showing good data a minute ago.

       * slateUsable() is the gate: JSON object, today's slate_date, a games
         array, not ok:false. Anything else (an HTML page never parses, a
         timeout never resolves) is a FAILED refresh.
       * holdRows() is the per sport fallback: a row that comes back missing, or
         empty when it had cards, or an MLB row the API itself flags degraded,
         keeps its last good cards for up to ROW_HOLD_MS. A row can genuinely
         empty out (last night's college finals leave at noon) so the hold ends;
         a restart is over long before it does.
       * The strip only shows the unavailable message after FAIL_LIMIT failed
         refreshes in a row AND with nothing recent enough to keep showing.
       * Past STALE_MS without a clean refresh the strip says so quietly (the
         label dot turns amber and reads "Delayed") instead of passing old
         scores off as current. Normal restarts never get that far.

     Pure functions, lifted by tests/ticker-last-known-good-test.cjs; the edge
     worker carries the same holdRows(). */
  var TK_ROW_KEYS = ['games', 'nfl_games', 'nba_games', 'nhl_games', 'cfb_games'];
  var TK_FAIL_LIMIT = 3;
  var TK_ROW_HOLD_MS = 5 * 60 * 1000;
  var TK_STALE_MS = 5 * 60 * 1000;
  var TK_LKG_MAX_MS = 30 * 60 * 1000;
  var TK_RETRY_MS = 15000;
  var TK_LKG_STORE = 'tmr.ticker.lkg.v1';
  ...
  function holdRows(p, lkg, now) {
    ...
    var rowAt = {};
    var held = [];
    var same = !!(lkg && lkg.payload && lkg.payload.slate_date === p.slate_date);
    TK_ROW_KEYS.forEach(function (key) {
      var fresh = p[key];
      var old = same ? lkg.payload[key] : null;
      var oldAt = same && lkg.rowAt ? lkg.rowAt[key] : null;
      var hadCards = Array.isArray(old) && old.length > 0;
      var suspect = !Array.isArray(fresh)
    ...
    return { payload: out, rowAt: rowAt, held: held };
  }

  var tkLkg = null;          // { payload, rowAt, goodAt }
  var tkGoodAt = null;       // when the strip last showed a clean refresh
  var tkFailures = 0;
  var tkRetryTimer = null;
  var tkFetching = false;

  function saveLkg() {
    try { window.localStorage.setItem(TK_LKG_STORE, JSON.stringify(tkLkg)); } catch (e) { /* storage is optional */ }
  }

  function storedLkg(today, now) {
    try {
      var s = JSON.parse(window.localStorage.getItem(TK_LKG_STORE) || 'null');
      if (s && s.payload && slateUsable(s.payload, today) && s.goodAt && now - s.goodAt <= TK_LKG_MAX_MS) return s;
    } catch (e) { /* storage is optional */ }
    return null;
  }

  function laneHasToday(lane, today) {
    return !!(lane && lane.getAttribute('data-slate-date') === today
      && lane.querySelector('.gm:not(.is-skel):not(.is-msg)'));
  }

  function markStale() {
    var t = el('.ticker'); if (!t) return;
    var stale = tkGoodAt != null && Date.now() - tkGoodAt > TK_STALE_MS
      && laneHasToday(el('.ticker .ticker-games'), slateDatePT());
    if (t.classList.contains('is-stale') !== stale) t.classList.toggle('is-stale', stale);
  }

  /* A usable slate: merge, remember, draw. */
  function acceptSlate(payload) {
    var now = Date.now();
    var base = tkLkg || storedLkg(payload.slate_date, now);
    var merged = holdRows(payload, base, now);
    /* Clean means every row came back on its own and the API does not call
       itself degraded. Anything less is shown, but does not count as fresh. */
    var clean = !merged.held.length && payload.degraded !== true;
    tkLkg = { payload: merged.payload, rowAt: merged.rowAt,
      goodAt: clean ? now : (base ? base.goodAt : tkGoodAt || now) };
    if (!clean) {
      tkFailures += 1;
    } else {
      tkFailures = 0;
    ...
    if (tkGoodAt == null) tkGoodAt = tkLkg.goodAt;
    renderTicker(merged.payload);
    markStale();
  }

  /* A refresh that produced nothing usable. */
  function slateFailed() {
    var now = Date.now();
    var today = slateDatePT();
    var lane = el('.ticker .ticker-games');
    tkFailures += 1;
    if (!laneHasToday(lane, today)) {
      var kept = (tkLkg && tkLkg.payload && tkLkg.payload.slate_date === today && now - tkLkg.goodAt <= TK_LKG_MAX_MS)
        ? tkLkg : storedLkg(today, now);
      if (kept) {
        tkLkg = kept;
        tkGoodAt = kept.goodAt;
        renderTicker(kept.payload);
        markStale();
        return;
      }
    }
    var recent = laneHasToday(lane, today) && tkGoodAt != null && now - tkGoodAt <= TK_LKG_MAX_MS;
    if (recent || (laneHasToday(lane, today) && tkGoodAt == null)) { markStale(); return; }
    if (tkFailures >= TK_FAIL_LIMIT) { renderTicker(null); return; }
    /* Nothing to show yet and not enough failures to say so: keep the loading
       lane and ask again soon rather than in ninety seconds. */
    if (!tkRetryTimer) {
      tkRetryTimer = setTimeout(function () { tkRetryTimer = null; if (!document.hidden) ticker(); }, TK_RETRY_MS);
    }
  }

  /* ------------------------------------------------ BREAKING MOMENTS

     Nima, 2026-09-14: the newest scoring play, turnover, home run or lead
     change "jumps to the front of that game's card for a few minutes", with
     how long ago it happened. The API sends the moment first in the card's
     insights with the wall clock time of the play (postgame/breakingMoments.js
     on the API); the window below matches the API's.

     Three things, each once per moment:
       * the card turns to the moment the first time it sees it, whatever line
         it was on (surfaceBreaking);
       * the carousel goes to the page holding a moment nobody has been shown
         yet before carrying on with its sequence (breakingPage);
       * the line wears a pulsing dot and "2m ago" until ten minutes have
         passed on the reader's clock, then reads as an ordinary line
         (refreshBreaking). */
  var BREAKING_WINDOW_MS = 10 * 60 * 1000;
  var BREAKING_SKEW_MS = 2 * 60 * 1000;
  var BREAKING_TICK_MS = 15000;
  var brkShown = {};
  var brkPaged = {};

  function brkAt(line) {
    var at = Date.parse(line.getAttribute('data-brk') || '');
    return isFinite(at) ? at : null;
  }

  function brkFresh(line, now) {
    var at = brkAt(line);
    return at != null && now - at <= BREAKING_WINDOW_MS && at - now <= BREAKING_SKEW_MS;
  }

  function brkKey(card, line) {
    return cardKey(card) + '|' + line.getAttribute('data-brk') + '|'
      + ((line.querySelector('b') || {}).textContent || '');
  }

  function refreshBreaking(root) {
    var now = Date.now();
    (root || document).querySelectorAll('.gm-in-l[data-brk]').forEach(function (line) {
      var chip = line.querySelector('.gm-ago');
      if (brkFresh(line, now)) {
        var mins = Math.floor(Math.max(0, now - brkAt(line)) / 60000);
        var text = mins < 1 ? 'Just now' : mins + 'm ago';
        if (!line.classList.contains('is-brk')) line.classList.add('is-brk');
        if (chip && chip.textContent !== text) chip.textContent = text;
      } else {
        line.classList.remove('is-brk');
        if (chip && chip.textContent) chip.textContent = '';
      }
    });
  }

  /* Turn a card to its newest fresh moment. `force` is the page arriving
     because of that moment, which shows it whether or not the card had already
     turned to it while nobody could see it. */
  function showBreaking(card, now, force) {
    var strip = card.querySelector('.gm-in');
    if (!strip) return;
    var lines = strip.querySelectorAll('.gm-in-l');
    for (var i = 0; i < lines.length; i++) {
      if (!lines[i].hasAttribute('data-brk') || !brkFresh(lines[i], now)) continue;
      var key = brkKey(card, lines[i]);
      if (brkShown[key] && !(force && !brkPaged[key])) return;
      brkShown[key] = true;
      ...
      strip.removeAttribute('data-was');
      return;
    }
  }

  function surfaceBreaking(root, force) {
    var now = Date.now();
    (root || document).querySelectorAll('.gm').forEach(function (card) { showBreaking(card, now, force); });
  }

  function markPaged(page) {
    if (!page || !page.querySelectorAll) return;
    var now = Date.now();
    page.querySelectorAll('.gm').forEach(function (card) {
      card.querySelectorAll('.gm-in-l[data-brk]').forEach(function (line) {
        if (brkFresh(line, now)) brkPaged[brkKey(card, line)] = true;
      });
    });
  }

  function breakingPage(track) {
    var now = Date.now();
    var current = track.children[tkPageIndex];
    for (var p = 0; p < track.children.length; p++) {
      var page = track.children[p];
      if (page === current) continue;
      var cards = page.querySelectorAll('.gm');
      for (var c = 0; c < cards.length; c++) {
        var lines = cards[c].querySelectorAll('.gm-in-l[data-brk]');
        for (var l = 0; l < lines.length; l++) {
          if (brkFresh(lines[l], now) && !brkPaged[brkKey(cards[c], lines[l])]) return page;
        }
      }
    }
    return null;
  ...
  /* A card's own dwell, in milliseconds. Pregame cards all share
     ...
     still inside the band the lines were written for, instead of one line
     forever. With the 2026-09-08 window of 40s that cap is 19.5s, up from
     11.5s, which is the whole of what "keep each highlight visible noticeably
     longer" means on a row that pages.
  ...
  function wireTickerControls() {
    if (tkWired) return;
    tkWired = true;
    /* How long ago a breaking moment happened, and when it stops being one. */
    setInterval(function () {
      var lane = el('.ticker .ticker-games');
      if (lane && !document.hidden) refreshBreaking(lane);
      markStale();
    }, BREAKING_TICK_MS);
  ...
  function ticker() {
    ...
    /* THE BAKE IS ONLY ADOPTED WHEN IT CAME FROM THIS BUILD.
       ...
       "USING THIS FILE'S OWN MARKUP" IS AN ASSUMPTION, AND IT CAN BE FALSE. The
       worker is deployed by wrangler and this file by a push to the site repo,
       so the two ship SEPARATELY and a window exists where the edge is baking
       the previous build's cards. Measured on production 2026-09-05, after the
       college scorebug shipped and before the worker was redeployed: the lane
       was adopted with 68 one-line college cards, and the reader sat looking at
       the old presentation for 91.6 SECONDS until the 90s refresh repainted it.
       That is the whole value of the change, invisible for a minute and a half,
       on the path almost every visitor takes.

       So the bake has to AGREE with what this build would draw. The scorebug
       sports are the test: this build renders them as .gm--bug, and a bake that
       has a college card which is not one was written by a different build.
       Cheap (one selector per scorebug sport), self-correcting (the moment the
       worker catches up the check passes and adoption resumes at no cost), and
       it costs nothing on the sports the two builds still agree about. */
    var bakeIsThisBuild = function () {
      for (var sp in SCOREBUG_SPORTS) {
        if (!Object.prototype.hasOwnProperty.call(SCOREBUG_SPORTS, sp)) continue;
        /* A started or finished game is a scorebug, an upcoming one is not
           (see renderTicker). The chip of an upcoming game carries no is-*
           state class, which is how either mismatch is recognised. */
        if (lane.querySelector('.gm--' + sp + ':not(.gm--bug) .st[class*="is-"]')) return false;
        if (lane.querySelector('.gm--' + sp + '.gm--bug .st:not([class*="is-"])')) return false;
      }
      return true;
    };

    if (!tickerAdoptChecked && baked === today &&
        lane.querySelector('.gm:not(.is-skel):not(.is-msg)') && bakeIsThisBuild()) {
      tickerAdoptChecked = true;
      tickerSettled = true;
      tickerSlateDate = today;
      lane.setAttribute('aria-busy', 'false');
      /* The edge baked its own last known good copy because the API was not
         answering cleanly: fetch the real thing shortly rather than in 90s. */
      var lkgAt = Number(lane.getAttribute('data-slate-lkg'));
      if (lane.hasAttribute('data-slate-lkg')) {
        tkGoodAt = lkgAt > 1e12 ? lkgAt : Date.now();
        setTimeout(function () { if (!document.hidden) ticker(); }, 5000);
      } else {
        tkGoodAt = Date.now();
      }
      refreshBreaking(lane);
    ...
    (firstSlate || slateFetch(false)).then(function (payload) {
      if (slateUsable(payload, slateDatePT())) { tkFetching = false; acceptSlate(payload); return; }
      /* Missing, malformed, an HTML page, or a slate from another day: bypass
         caches and ask the server once more. What happens when that fails too is
         slateFailed()'s call, not an immediate unavailable message. */
      slateFetch(true).then(function (retry) {
        tkFetching = false;
        if (slateUsable(retry, slateDatePT())) { acceptSlate(retry); return; }
        slateFailed();
      });
    });
  }
  ...
  function livePicks(users, emptyText) {
    ...
    rows = rows.slice(0, 6);
    if (!rows.length) {
      // A sport tab with nothing settled says so; it never keeps another tab's rows.
      if (emptyText) card.innerHTML = '<div class="pk"><span class="bd"><span class="ln">' + esc(emptyText) + '</span></span></div>';
      return;
    }
  ...
  /* LIVE_PICKS_20260915. The card used to render /home-bootstrap's
     trend_highlights: the 24 members with the most lifetime net units and
     their graded picks. Everybody else's picks never reached it, so it sat on
     Sunday's rows while newer ones existed; its tabs had no handler at all, and
     nothing re-read it inside an open tab. It now renders live_picks, the API's
     newest settled public picks from every member, six per tab (All and each
     sport group), and re-reads /users/live-picks every 60s while visible. The
     API drops that cache on every grading run, so a newly settled pick shows
     up on the next read with no deploy or bake. A backend that predates
     live_picks keeps the old trend_highlights rendering, filtered by tab. */
  var LP_REFRESH_MS = 60000;
  var lp = { data: null, legacy: null, tab: 'all', timer: null, bound: false };
  var LP_LEGACY_TAB = {
    mlb: function (k) { return k === 'baseball_mlb'; },
    nfl: function (k) { return k === 'americanfootball_nfl'; },
    soccer: function (k) { return /^soccer(_|$)/.test(k); }
  };

  function lpTabs() {
    return document.querySelectorAll('.board .card:nth-child(1) .tabs .tab');
  }
  function lpTabKey(btn) {
    return String(btn.getAttribute('data-tab') || btn.textContent || '').trim().toLowerCase();
  }
  function lpRender() {
    var tab = lp.tab;
    var label = tab.toUpperCase();
    lpTabs().forEach(function (b) {
      if (lpTabKey(b) === tab) label = b.textContent.trim();
    });
    ...
    if (lp.data) {
      ...
      var users = !test ? lp.legacy : lp.legacy.map(function (u) {
        return Object.assign({}, u, { picks: (u.picks || []).filter(function (p) { return test(String(p.sport_key || '')); }) });
      });
      livePicks(users, empty);
    }
  }
  function lpBindTabs() {
    if (lp.bound) return;
    lp.bound = true;
    lpTabs().forEach(function (btn) {
      btn.addEventListener('click', function () {
        lp.tab = lpTabKey(btn);
        lpTabs().forEach(function (b) { b.classList.toggle('on', b === btn); });
        lpRender();
      });
    });
  }
  function lpRefresh() {
    if (document.hidden) return;
    j('/users/live-picks', 8000).then(function (d) {
      if (d && d.live_picks && d.live_picks.all) applyLivePicks(d.live_picks);
    });
  }
  function applyLivePicks(data) {
    lp.data = data;
    lpBindTabs();
    lpRender();
    if (!lp.timer) {
      lp.timer = setInterval(lpRefresh, LP_REFRESH_MS);
      document.addEventListener('visibilitychange', function () { if (!document.hidden) lpRefresh(); });
    }
  }
  function applyLegacyPicks(users) {
    lp.legacy = users;
    lpBindTabs();
    lpRender();
  }
  ...
  /* ---------- 3. LEADERBOARD --------------------------------------------- */
  /* CANONICAL_RANKING_20260914: this card lists OFFICIALLY RANKED members only,
     in the API's official order, and prints each member's official rank. It
     used to re-sort by net units and print the row index, which numbered
     losing records. With no official ranks issued it says so. */
  function leaderboard(users) {
    var body = el('.board .card:nth-child(2) .body'); if (!body || !users) return;
    var ranked = users.filter(function (u) { return num(u.official_rank) > 0; })
                      .sort(function (a, b) { return num(a.official_rank) - num(b.official_rank); });
    if (!ranked.length) {
      body.innerHTML = '<div class="lbr lb-empty"><span class="nm"><b>No official ranks issued yet</b>' +
        '<span>A TMR rank needs 25 graded picks, recent activity, positive units and ROI, and 3 qualified handicappers.</span></span></div>';
      return;
    }
    body.innerHTML = ranked.slice(0, 10).map(function (u) {
  ...
  function compNeutral(username, id) {
    ...
    return '<img class="comp-av" src="' + esc(((window.CONFIG && window.CONFIG.api && window.CONFIG.api.baseUrl) || 'https://trustmyrecord-api.onrender.com/api') + '/users/' + encodeURIComponent(key || '') + '/avatar') +
  ...
  function boot() {
    ...
    (firstBoot || j('/users/home-bootstrap', 6000)).then(function (d) {
      ...
      if (d.live_picks && d.live_picks.all) applyLivePicks(d.live_picks);
      else if (d.trend_highlights && d.trend_highlights.length) applyLegacyPicks(d.trend_highlights);
      lwReveal('tmr-lw-b1');
  ...
  function legacyBoot() {
    ...
    j('/users/live-picks', 8000).then(function (lpd) {
      if (lpd && lpd.live_picks && lpd.live_picks.all) {
        applyLivePicks(lpd.live_picks);
        lwReveal('tmr-lw-b1');
        return;
      }
      j('/users/trend-highlights').then(function (d) {
        if (d && d.users && d.users.length) applyLegacyPicks(d.users);
        lwReveal('tmr-lw-b1');
      });
    });
    ...
    j('/users/leaderboard?sortBy=rank&limit=10', 8000).then(function (d) {
  ...
  /* Not while a request is still out or a retry is queued: a backend that is
     restarting answers late, and the lane keeps its loading state until
     slateFailed() has seen enough failures to say otherwise. */
  function tickerBackstop() {
    if (tkFetching || tkRetryTimer) { setTimeout(tickerBackstop, TK_RETRY_MS); return; }
    var lane = document.querySelector('.ticker .ticker-games');
    if (lane && !lane.querySelectorAll('.gm:not(.is-skel)').length &&
