(function() {
    ...
    function formatLine(value, options) {
        ...
        let text = n.toFixed(2).replace(/0+$/, '').replace(/\.$/, '');
        if (options && options.signed && n > 0) text = '+' + text;
        return text;
    }
    ...
    function calculateStakeValues(mode, amount, odds) {
        ...
        if (!Number.isFinite(units) || units <= 0 || !Number.isFinite(price) || price === 0) {
            return { risk_units: 0, win_units: 0 };
        }
        if (stakeMode === 'to_win') {
            return {
                risk_units: roundStakeUnits(price < 0 ? units * Math.abs(price) / 100 : units * 100 / price),
                win_units: roundStakeUnits(units)
            };
        }
        return {
            risk_units: roundStakeUnits(units),
            win_units: roundStakeUnits(price < 0 ? units * 100 / Math.abs(price) : units * price / 100)
        };
    }
    ...
    function renderStakeSummaryHtml(values) {
        const riskText = formatStakePreviewUnits(values.risk_units);
        const winText = formatStakePreviewUnits(values.win_units);
        return '<span class="tmr-ticket-stake-summary-cell">' +
            '<span class="tmr-ticket-stake-summary-label">Risk</span>' +
            '<strong class="tmr-ticket-stake-summary-value">' + escapeHtml(riskText + ' ' + formatStakeUnitLabel(values.risk_units)) + '</strong>' +
            '</span>' +
            '<span class="tmr-ticket-stake-summary-divider" aria-hidden="true"></span>' +
            '<span class="tmr-ticket-stake-summary-cell">' +
            '<span class="tmr-ticket-stake-summary-label">To Win</span>' +
            '<strong class="tmr-ticket-stake-summary-value">' + escapeHtml(winText + ' ' + formatStakeUnitLabel(values.win_units)) + '</strong>' +
            '</span>';
    }

    function getStakeAmountInput() {
    ...
    function updateStakeModePreview(commit) {
        ...
        ['unitsStakePreview', 'ttSlipStakePreview'].forEach(function(id) {
            ...
            if (values.risk_units > 0 && values.win_units > 0) {
                ...
                preview.textContent = 'Risk / To Win preview updates after odds are entered.';
            }
        });
    }
            ...
            try {
                ...
                if (shouldTraceBoardRequest) {
                    recordBoardEvent('api_success', {
                        ...
                        duration_ms: elapsedMs(requestStartedAt),
    ...
    async function fetchMarketBoardFast(sportKey, forceRefresh) {
        ...
        if (!forceRefresh) {
            ...
            if (cached) {
                recordBoardEvent('board_cache_hit', {
                    sport_key: sportKey,
                    duration_ms: elapsedMs(boardFetchStartedAt),
        ...
        const request = (async function() {
            try {
                recordBoardEvent('board_fetch_started', { sport_key: sportKey, force_refresh: !!forceRefresh });
                ...
                recordBoardEvent(forceRefresh ? 'board_refresh_success' : 'board_fetch_success', {
                    sport_key: sportKey,
                    source: 'direct',
                    direct_duration_ms: elapsedMs(directStartedAt),
                    duration_ms: elapsedMs(boardFetchStartedAt),
                ...
                const apiClientStartedAt = nowMs();
                ...
                recordBoardEvent(forceRefresh ? 'board_refresh_fallback_success' : 'board_fetch_fallback_success', {
                    sport_key: sportKey,
                    source: 'api_client',
                    api_client_duration_ms: elapsedMs(apiClientStartedAt),
                    duration_ms: elapsedMs(boardFetchStartedAt),
    ...
    function injectStyles() {
        ...
        style.textContent = [
            ...
            '#picks #gamesListContainer .tmr-market-card .tmr-market-head{grid-template-columns:1fr!important;}',
            '#picks #gamesListContainer .tmr-market-card .tmr-market-side{display:grid!important;grid-template-columns:1fr!important;width:100%!important;margin-top:14px!important;}',
            '#picks #gamesListContainer .tmr-market-card .tmr-primary-market-grid{width:100%!important;max-width:none!important;min-width:0!important;}',
            ...
            '.tmr-team-row{display:grid;grid-template-columns:30px 54px minmax(0,1fr);align-items:center;gap:10px;flex-wrap:nowrap;color:#f8fafc;min-width:0;}',
            ...
            '#picks #gamesListContainer .tmr-market-card{min-width:360px;}',
    ...
    function renderBoardLoading(container, sport) {
        ...
        container.innerHTML = '<div class="tmr-loading-slate">' +
            ...
            '<div class="tmr-empty-state" style="margin:0 0 14px;">Loading live odds. This may take a few seconds.</div>' +
    ...
    function normalizeBoardResponse(response, sport) {
        ...
        const hasPricedMarkets = function(game) {
            return game && game.has_sportsbook_odds && (game.market_groups || []).some(function(group) {
    ...
    function buildFallbackBoardGames(games, sportKey) {
        return (games || []).map(function(game, index) {
            ...
            if (awaySpread) fullGameItems.push(createFallbackOption(game, index, 'Full Game', 'spreads', game.away_team, game.away_team + ' ' + formatLine(awaySpread.point, { signed: true }), awaySpread.price, awaySpread.point));
            if (homeSpread) fullGameItems.push(createFallbackOption(game, index, 'Full Game', 'spreads', game.home_team, game.home_team + ' ' + formatLine(homeSpread.point, { signed: true }), homeSpread.price, homeSpread.point));
            if (over) fullGameItems.push(createFallbackOption(game, index, 'Game Total', 'totals', 'Over', 'Over ' + formatLine(over.point), over.price, over.point));
            if (under) fullGameItems.push(createFallbackOption(game, index, 'Game Total', 'totals', 'Under', 'Under ' + formatLine(under.point), under.price, under.point));
                ...
                marketGroups.push({
                    ...
                    items: teamTotals.outcomes.map(function(outcome) {
                        return createFallbackOption(
                            ...
                            'team_totals',
                            outcome.name,
                            outcome.point != null ? (outcome.name + ' ' + formatLine(outcome.point)) : outcome.name,
                            outcome.price,
                ...
                marketKeys.forEach(function(marketKey) {
                    ...
                    market.outcomes.forEach(function(outcome) {
                        ...
                        if (isTotal && line != null) {
                            label = outcome.name + ' ' + formatLine(line);
                        } else if (!isMoneyline && line != null) {
                            label = outcome.name + ' ' + formatLine(line, { signed: true });
                        } else if (isMoneyline) {
            ...
            if (sportKey === 'baseball_mlb' && (f5H2h || f5Spreads || f5Totals)) {
                ...
                const first5Items = [];
                if (f5AwaySpread) first5Items.push(createFallbackOption(game, index, 'First 5', 'f5_spreads', game.away_team, game.away_team + ' ' + formatLine(f5AwaySpread.point, { signed: true }), f5AwaySpread.price, f5AwaySpread.point, bookmaker ? bookmaker.title : 'Sportsbook feed'));
                if (f5HomeSpread) first5Items.push(createFallbackOption(game, index, 'First 5', 'f5_spreads', game.home_team, game.home_team + ' ' + formatLine(f5HomeSpread.point, { signed: true }), f5HomeSpread.price, f5HomeSpread.point, bookmaker ? bookmaker.title : 'Sportsbook feed'));
                ...
                if (f5Over) first5Items.push(createFallbackOption(game, index, 'First 5', 'f5_totals', 'Over', 'F5 Over ' + formatLine(f5Over.point), f5Over.price, f5Over.point, bookmaker ? bookmaker.title : 'Sportsbook feed'));
                if (f5Under) first5Items.push(createFallbackOption(game, index, 'First 5', 'f5_totals', 'Under', 'F5 Under ' + formatLine(f5Under.point), f5Under.price, f5Under.point, bookmaker ? bookmaker.title : 'Sportsbook feed'));
    ...
    function getCanonicalLogoEntry(teamName, sportKey) {
        ...
        const mapsToCheck = [];
        if (CANONICAL_TEAM_LOGOS[normalizedSport]) mapsToCheck.push(CANONICAL_TEAM_LOGOS[normalizedSport]);
        Object.keys(CANONICAL_TEAM_LOGOS).forEach(function(leagueKey) {
            const sportMap = CANONICAL_TEAM_LOGOS[leagueKey];
            if (mapsToCheck.indexOf(sportMap) === -1) mapsToCheck.push(sportMap);
        });
        for (let mapIndex = 0; mapIndex < mapsToCheck.length; mapIndex += 1) {
            const sportMap = mapsToCheck[mapIndex];
    ...
    async function selectSportAndShowGames(sport) {
        ...
        var slowMessageTimer = window.setTimeout(function() {
            if (requestId !== latestBoardRequestId) return;
            var c = document.getElementById('gamesListContainer');
            if (!c) return;
            if (c.querySelector('.tmr-board-loading') || c.querySelector('.tmr-loading-slate') || c.textContent.indexOf('Loading') !== -1) {
                recordBoardEvent('board_slow_loading_message', {
                    sport: sport,
                    sport_key: sportKey,
                    elapsed_ms: elapsedMs(selectionStartedAt)
                });
                var slate = c.querySelector('.tmr-loading-slate');
                if (slate && !slate.querySelector('.tmr-loading-slow-note')) {
                    slate.insertAdjacentHTML('afterbegin', '<div class="tmr-empty-state tmr-loading-slow-note" style="margin-bottom:14px;">Still loading live odds. The feed is responding slower than normal.</div>');
                }
            }
        }, 4500);
        ...
        var loadingTimeoutTimer = window.setTimeout(function() {
            if (requestId !== latestBoardRequestId) return;
            if (state.lastBoardRenderSport === sport && state.lastBoardRenderAt > Date.now() - 10000) return;
            var c = document.getElementById('gamesListContainer');
            if (!c) return;
            if (c.querySelector('.tmr-board-loading') || c.querySelector('.tmr-loading-slate') || c.textContent.indexOf('Loading') !== -1) {
                recordBoardEvent('board_loading_timeout', {
                    sport: sport,
                    sport_key: sportKey,
                    elapsed_ms: elapsedMs(selectionStartedAt)
                });
                c.innerHTML = '<div class="tmr-empty-state">Odds are temporarily unavailable. Please try again shortly. ' +
        ...
        try {
            ...
            recordBoardEvent('board_selection_complete', {
                sport: sport,
                sport_key: sportKey,
                duration_ms: elapsedMs(selectionStartedAt),
                snapshot: snapshotBoardPayload(response)
            });
    ...
    async function lockInPick() {
        ...
        try {
            ...
            const payload = {
                ...
                bet_type: option.market_type === 'team_totals' ? 'team_total' : option.market_type,
    ...
    function getTeamTotalSubmitMeta(option) {
        ...
        const side = parsed ? parsed[2].toLowerCase() : (/\bunder\b/i.test(visible) ? 'under' : (/\bover\b/i.test(visible) ? 'over' : ''));
        const game = option.game || {};
        const home = game.home_team || option.home_team || '';
        const away = game.away_team || option.away_team || '';
        const norm = function (value) { return String(value || '').trim().toLowerCase(); };
        let opponent = '';
        if (team && (norm(team) === norm(home) || norm(home).indexOf(norm(team)) !== -1 || norm(team).indexOf(norm(home)) !== -1)) opponent = away;
        else if (team && (norm(team) === norm(away) || norm(away).indexOf(norm(team)) !== -1 || norm(team).indexOf(norm(away)) !== -1)) opponent = home;
        return {
            team_name: team || '',
            opponent_name: opponent || '',
            side: side || '',
            league: option.sport_key || ''
        };
    }
    ...
    function renderPickCard(pick) {
        ...
        const stakeText = formatStakeDisplay(pick);
        ...
        return '<div' + pickIdAttrs + ' style="background:#22262e;border-radius:12px;padding:16px;margin-bottom:12px;border-left:4px solid ' + statusColor + ';">' +
            ...
            (stakeText ? '<div style="font-size:12px;color:#dbeafe;margin-top:6px;font-weight:700;">' + escapeHtml(stakeText) + '</div>' : '') +
            '</div>' +
    ...
    function boot() {
        ...
        window.selectGameBet = function(gameIndex, betType, team, line, odds, awayTeam, homeTeam) {
            try {
                ...
                // Team-totals legacy renderer (sportsbook/index.html _ttRenderRows)
                // overwrites window.TMR.currentGames with the TT board but
                // leaves state.currentBoard pointing at the previous Full-Game
                // board. Index-based lookup against state.currentBoard then
                // resolves to the wrong game, so the team-name in the selection
                // fails backend validation against the wrong game's teams.
                // Prefer the actively-rendered board and require home/away args
                // to match; otherwise fall back to the other source.
                // other source. Full-Game clicks never touch this bridge
                // (they go through tmrSelectOption directly), so this only
                // affects team-totals.
                ...
                var pickGame = null;
                if (matchesTeams(tmrGames[gameIndex])) pickGame = tmrGames[gameIndex];
                else if (matchesTeams(board[gameIndex])) pickGame = board[gameIndex];
                else {
                    var foundInTmr = tmrGames.find(matchesTeams) || null;
                    var foundInBoard = board.find(matchesTeams) || null;
                    pickGame = foundInTmr || foundInBoard || tmrGames[gameIndex] || board[gameIndex] || null;
                }
                var game = pickGame;
                if (!game) {
                ...
                switch (betType) {
                    ...
                    case 'firsthalfml':
                        ...
                        selection = teamRaw;
                        selectionLabel = teamRaw + ' 1H ML';
                        break;
                    case 'firsthalfspread':
                        marketType = 'first_half_spreads'; groupLabel = 'First Half';
                        selection = teamRaw;
                        selectionLabel = teamRaw + ' 1H' + (lineDispSigned ? ' ' + lineDispSigned : '');
                        break;
                    case 'firsthalfover':
                        marketType = 'first_half_totals'; groupLabel = 'First Half';
                        selection = 'Over';
                        selectionLabel = '1H Over' + (lineDisp ? ' ' + lineDisp : '');
                        break;
                    case 'firsthalfunder':
                        marketType = 'first_half_totals'; groupLabel = 'First Half';
                        selection = 'Under';
                        selectionLabel = '1H Under' + (lineDisp ? ' ' + lineDisp : '');
                        break;
                    case 'secondhalfml':
                        marketType = 'second_half_h2h'; groupLabel = 'Second Half';
                        selection = teamRaw;
                        selectionLabel = teamRaw + ' 2H ML';
                        break;
                    case 'secondhalfspread':
                        marketType = 'second_half_spreads'; groupLabel = 'Second Half';
                        selection = teamRaw;
                        selectionLabel = teamRaw + ' 2H' + (lineDispSigned ? ' ' + lineDispSigned : '');
                        break;
                    case 'secondhalfover':
                        marketType = 'second_half_totals'; groupLabel = 'Second Half';
                        selection = 'Over';
                        selectionLabel = '2H Over' + (lineDisp ? ' ' + lineDisp : '');
                        break;
                    case 'secondhalfunder':
                        marketType = 'second_half_totals'; groupLabel = 'Second Half';
                        selection = 'Under';
...
(function () {
    ...
    function injectStyles() {
        ...
        var css = ''
            ...
            + '#tmrQuickBet .qb-uinput{width:58px;min-width:58px;height:42px;border:none;background:transparent;'
            + 'padding:0 4px;color:#fff;text-align:center;font-size:16px;line-height:1.3;font-weight:800;font-family:Barlow,Inter,sans-serif;'
            + 'overflow:visible;-webkit-appearance:none;-moz-appearance:textfield;}'
