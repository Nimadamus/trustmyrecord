...
(function () {
  ...
  /* A top-level trigger is dropped on the page it is named for: on
     /sportsbook/ the word SPORTSBOOK is the page you are already reading, not
     a destination. Only the trigger goes; the menu's other routes are
     unchanged everywhere else, and no other page loses anything, because the
     test is the menu's OWN primary route (list[0]) matching the current one.
     Requested 2026-08-31. */
  function menu(label, list, alsoCurrent) {
    if (list.length && isCurrent(list[0][0])) return '';
