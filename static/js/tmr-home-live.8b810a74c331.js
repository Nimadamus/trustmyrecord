  ...
  function renderTicker(payload) {
    ...
    espnRows.forEach(function (row) {
      row.games.forEach(function (g) {
        /* The scorebug rows draw themselves. Everything else keeps the one-line
           card it has always had.

           AN UPCOMING GAME IS NOT A SCOREBUG (Nima, 2026-09-14: the Syracuse at
           Pitt card "looks totally different ... doesn't look like the others").
           With no score the scorebug is two empty score columns under a header,
           beside one-line MLB and NFL cards. A college game that has not
           kicked off draws the same one-line card as every other upcoming game;
           the scorebug is for a game with a score to show. */
        if (SCOREBUG_SPORTS[row.key] && g.status !== 'scheduled') { html += scorebugCard(g, row.key); return; }
