...
/**
 * Generate sample games when API is unavailable
 */
function getSampleGames(sport) {
    const now = new Date();
        ...
        'NFL': [
            {
                ...
                odds: {
                    ...
                    totals: { over: -110, under: -110, point: 47.5 }
                }
            },
            {
                id: 'sample_nfl_2',
                ...
                odds: {
                    moneyline: { home: -170, away: +150 },
                    spread: {
                        home: { point: -4.5, price: -108 },
                        away: { point: +4.5, price: -112 }
                    },
                    totals: { over: -112, under: -108, point: 44.5 }
                }
            }
        ],
        'NBA': [
            {
                ...
                odds: {
                    ...
                    totals: { over: -110, under: -110, point: 221.5 }
                }
            },
            {
                id: 'sample_nba_2',
                ...
                odds: {
                    ...
                    spread: {
                        home: { point: +4, price: -108 },
                        away: { point: -4, price: -112 }
                    },
                    totals: { over: -112, under: -108, point: 228.5 }
                }
            }
        ],
        'NHL': [
            {
                ...
                odds: {
                    ...
                    spread: {
                        home: { point: -1.5, price: +165 },
                        away: { point: +1.5, price: -185 }
                    },
                    totals: { over: -115, under: -105, point: 6.5 }
                }
            }
        ],
        'MLB': [
            {
                ...
                odds: {
                    moneyline: { home: -145, away: +130 },
                    spread: {
                        home: { point: -1.5, price: +140 },
                        away: { point: +1.5, price: -160 }
                    },
                    totals: { over: -110, under: -110, point: 8.5 }
                }
            }
        ],
        'NCAAF': [
            {
                ...
                odds: {
                    moneyline: { home: +160, away: -180 },
                    spread: {
                        home: { point: +4.5, price: -110 },
                        away: { point: -4.5, price: -110 }
                    },
                    totals: { over: -110, under: -110, point: 52.5 }
                }
            }
        ],
        'NCAAB': [
            {
                ...
                odds: {
                    ...
                    totals: { over: -110, under: -110, point: 148.5 }
                }
            }
        ]
    };

    return sampleGames[sport] || [];
}
...
async function loadGames() {
    ...
    try {
        ...
        console.log('⚠️ Falling back to sample games...');

        // Use sample games as fallback
        const sampleGames = getSampleGames(selectedSport);

        if (!sampleGames || sampleGames.length === 0) {
            gamesGrid.innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--text-muted);">
                    <p style="color: var(--neon-yellow); margin-bottom: 10px;">⚠️ API Unavailable</p>
                    <p>No sample games available for ${selectedSport}</p>
        ...
        // Filter sample games by bet type
        ...
        if (filteredGames.length === 0) {
            gamesGrid.innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--text-muted);">
                    <p style="color: var(--neon-yellow); margin-bottom: 10px;">⚠️ API Unavailable - Showing Sample Data</p>
                    <p>No sample games available with ${selectedBetType} odds for ${selectedSport}</p>
                </div>
            `;
            return;
        }
        ...
        // Add sample data warning banner
        const warningBanner = `
            <div style="background: linear-gradient(135deg, rgba(255, 193, 7, 0.15), rgba(255, 152, 0, 0.15));
                ...
                <strong>⚠️ Sample Data Mode</strong><br>
                <small>API unavailable. Showing sample games for demonstration purposes.</small>
            </div>
        `;

        // Render game cards (same logic as above)
        const gameCards = filteredGames.map((game, index) => {
            ...
            if (selectedBetType === 'spread' && game.odds.spread) {
                ...
                oddsDisplay = `
                    <div class="game-odds">
                        <div class="total">Over ${game.odds.totals.point} (${formatOdds(game.odds.totals.over)})</div>
                        <div class="total">Under ${game.odds.totals.point} (${formatOdds(game.odds.totals.under)})</div>
                    </div>
                `;
            } else if (selectedBetType === 'prop') {
                oddsDisplay = `
        ...
        gamesGrid.innerHTML = warningBanner + gameCards;
        console.log(`✅ Loaded ${filteredGames.length} sample ${selectedSport} games with ${selectedBetType} odds`);
    }
}
