...
(function () {
  ...
  function applyGameFile() {
    ...
    for (var pk in byPk) {
      ...
      decorate(lane, pk, byPk[pk].url);
    }

    /* When the API sent a map, it IS the answer — return whether or not a card
       was marked. This function runs before the strip has painted as well as
       after it, and on that first call nothing matches yet; falling through to
       the name-matching fallback there marked the FEATURED game instead of the
       game the map named, which on a day when those differ is simply the wrong
       card. A later call marks the right one once the cards exist. */
    if (haveMap) return;
