...
(function (global) {
  ...
  SimApp.prototype.render = function (d) {
    ...
    /**
     * WHAT CHANGED, at the top, because it is the reason the reader ran again.
     *
     * Shown only when a previous result was pinned and this one is a different
     * run of the same matchup. It states what moved and whether the move clears
     * the sampling error of the two runs behind it -- a difference inside the
     * noise is reported as noise rather than as an effect.
     */
    if (this.pinned && this.pinned.result && this.pinned.result !== d) {
      var cmp = whatChanged(this.pinned.result, d);
      var wrap = el('div', 'panel changed');
      wrap.appendChild(el('h3', '', 'What changed'));
      wrap.appendChild(el('p', 'dim',
        'From ' + this.pinned.label + ' to ' + scenarioLabel({
          home: (this.scenario && this.scenario.home) || [],
          away: (this.scenario && this.scenario.away) || [],
          neutral: this.state && this.state.venue === 'neutral',
        }) + '.'));
      wrap.appendChild(table(
        [{ h: 'Measure', fmt: function (r) { return r.what; } },
          { h: 'Pinned', fmt: function (r) { return r.before; } },
          { h: 'This run', fmt: function (r) { return r.after; } },
          { h: 'Change', fmt: function (r) { return r.change; } }],
        cmp.rows,
      ));
      wrap.appendChild(el('p', cmp.meaningful ? 'note' : 'note warn', cmp.note));
      box.appendChild(wrap);
    }
  ...
  /**
   * Compare two runs of the same matchup and say what moved -- and, just as
   * importantly, whether the move is bigger than the noise.
   *
   * TWO KINDS OF NUMBER LIVE ON THIS PAGE AND THEY NEED DIFFERENT TREATMENT.
   *
   * Win probability is NOT counted from the replays. It comes from the
   * calibrated model, and it is identical across seeds and sample sizes -- the
   * same matchup returns 0.414 at 200 replays and at 8,000. So it carries no
   * sampling error, and any movement in it is the model responding to an input
   * you changed. Calling a real 0.5pp shift "noise" would be wrong.
   *
   * The projected score and total ARE averages over the replays, and they do
   * wander: the same matchup returned 111.4 and 111.9 on two runs. For those the
   * standard error of the mean is sd/sqrt(n), and for a difference between two
   * runs the errors add in quadrature. A move inside two of those is reported as
   * indistinguishable from noise rather than dressed up as an effect.
   *
   * Nothing here invents a cause. It states which inputs the reader changed and
   * what the model did with them.
   */
  function probStdErr(p, n) {
    if (!(n > 0)) return 0;
    var q = Math.max(0, Math.min(1, p || 0));
    return Math.sqrt((q * (1 - q)) / n);
  }

  function simsOf(r) {
    return (r && r.meta && r.meta.simulations) || 0;
  }

  function scenarioLabel(sc) {
    if (!sc) return 'current conditions';
    var bits = [];
    if (sc.home && sc.home.length) bits.push(sc.home.length + ' home out');
    if (sc.away && sc.away.length) bits.push(sc.away.length + ' away out');
    if (sc.homeMinutes) bits.push('home minutes capped');
    if (sc.awayMinutes) bits.push('away minutes capped');
    if (sc.neutral) bits.push('neutral site');
    return bits.length ? bits.join(', ') : 'current conditions';
  }

  /**
   * @returns {{rows: Array, meaningful: boolean, note: string}}
   */
  function whatChanged(a, b) {
    ...
    var wpA = (a.projection && a.projection.win_probability && a.projection.win_probability.home) || 0;
    var wpB = (b.projection && b.projection.win_probability && b.projection.win_probability.home) || 0;
    var wpDiff = wpB - wpA;

    // The sampling band belongs to the SCORE, which is an average over replays.
    // Win probability has none: it is deterministic given the inputs.
    var sdA = (a.projection && a.projection.total && a.projection.total.sd) || 0;
    var sdB = (b.projection && b.projection.total && b.projection.total.sd) || 0;
    var se = Math.sqrt((nA ? (sdA * sdA) / nA : 0) + (nB ? (sdB * sdB) / nB : 0));
    var meaningful = Math.abs(wpDiff) > 1e-9;

    var pct = function (v) { return (v * 100).toFixed(1) + '%'; };
    var signed = function (v, dp) {
      var n = Number(v || 0);
      return (n > 0 ? '+' : '') + n.toFixed(dp === undefined ? 1 : dp);
    };

    rows.push({
      what: 'Home win probability',
      before: pct(wpA),
      after: pct(wpB),
      change: signed(wpDiff * 100) + 'pp',
    });

    var scoreA = (a.projection && a.projection.projected_score) || {};
    var scoreB = (b.projection && b.projection.projected_score) || {};
    if (scoreA.home !== undefined && scoreB.home !== undefined) {
      rows.push({
        what: 'Projected score',
        before: scoreA.away + ' - ' + scoreA.home,
        after: scoreB.away + ' - ' + scoreB.home,
        change: signed((scoreB.home - scoreA.home) - (scoreB.away - scoreA.away)) + ' on the margin',
      });
    }

    var spA = (a.projection && a.projection.spread) || {};
    var spB = (b.projection && b.projection.spread) || {};
    if (spA.home !== undefined && spB.home !== undefined) {
      rows.push({
        what: 'Projected spread',
        before: signed(spA.home),
        after: signed(spB.home),
        change: signed(spB.home - spA.home),
      });
    }

    var tA = (a.projection && a.projection.total) || {};
    var tB = (b.projection && b.projection.total) || {};
    if (tA.mean !== undefined && tB.mean !== undefined) {
      rows.push({
        what: 'Projected total',
        before: tA.mean.toFixed(1),
        after: tB.mean.toFixed(1),
        change: signed(tB.mean - tA.mean),
      });
    }

    var totalBand = (2 * se).toFixed(2);
    var note = meaningful
      ? 'The win probability moved because an input you changed moved it. That figure '
        + 'comes from the calibrated model rather than being counted from the replays, '
        + 'so it does not wander between runs and this shift is real. The projected '
        + 'score and total are averages over the replays and carry about '
        + totalBand + ' points of sampling noise at these sample sizes, so read small '
        + 'movements in those with that in mind.'
      : 'The win probability did not move at all. It is computed from the calibrated '
        + 'model rather than counted from the replays, so it changes only when an input '
        + 'to the model changes -- and nothing you changed reached it. Any difference '
        + 'you see in the score or total below is the replays wandering, worth about '
        + totalBand + ' points at these sample sizes.';

    return { rows: rows, meaningful: meaningful, note: note, stderr: se };
  }
  ...
  function actionBar(app, opts) {
    ...
    /**
     * PIN, then change something, then run again.
     *
     * Kept to one button because the second half of the comparison is just the
     * ordinary Run control: pin a result, adjust whatever you like, and the next
     * run is diffed against what you pinned. Nothing is stored anywhere but this
     * tab.
     */
    add(app.pinned ? 'Pinned - run again to compare' : 'Pin for comparison', '', function () {
      if (!app.lastResult) return;
      app.pinned = app.pinned ? null : {
        result: app.lastResult,
        label: scenarioLabel({
          home: (app.scenario && app.scenario.home) || [],
          away: (app.scenario && app.scenario.away) || [],
          neutral: app.state && app.state.venue === 'neutral',
        }),
      };
      said.textContent = app.pinned
        ? 'Pinned. Change something and run again to see what moved.'
        : 'Comparison cleared.';
      if (app.renderActions) app.renderActions();
    }, 'Hold this result, change an input, and compare');

    /**
     * The numbers behind the page, for anyone who wants to check them or take
     * them elsewhere. Built from the result already in memory, so it costs
     * nothing and cannot disagree with what is displayed.
     */
    add('Download JSON', '', function () {
      if (!app.lastResult) return;
      var payload = JSON.stringify(app.lastResult, null, 2);
      var blob = new global.Blob([payload], { type: 'application/json' });
      var url = global.URL.createObjectURL(blob);
      var link = global.document.createElement('a');
      link.href = url;
      link.download = (app.sport || 'tmr') + '-simulation-'
        + ((app.lastResult.meta && app.lastResult.meta.seed) || 'result') + '.json';
      global.document.body.appendChild(link);
      link.click();
      global.document.body.removeChild(link);
      global.setTimeout(function () { global.URL.revokeObjectURL(url); }, 2000);
      said.textContent = 'Downloaded the full result, including the seed that produced it.';
