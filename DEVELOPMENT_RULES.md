...
- add a second featured-game system. The hand-edited `GAME_OF_THE_WEEK` dict was removed on
  ...
  **Superseded for every sport on Sep 14, 2026 (mlb, ncaaf, nfl, soccer, tennis, and /today/):** a hand built feature page registers itself with `<meta name="tmr-featured">` tags on the next push. For those sports that
...
Requirements:
1. Every clickable price button routes through the shared selection state. Either `window.selectGameBet(...)` (board games indexed in `window.TMR.currentGames`) OR, for feeds NOT in `currentGames` (alt lines, and any future independent feed), a dedicated handler that calls `window.TMR._ttPopulateSlip(...)` directly with the game info embedded on the card (see `window.selectAltBet` + the `[data-alt-pick="1"]` delegated handler). NEVER render a bare `<div>`/`<button>` price with no handler.
2. Alt lines are pickable game-line markets, NOT props: `alt_spreads` → `betType:'spread'`, `marketType:'spread'`, sport-aware label (Alt Run Line / Alt Puck Line / Alt Spread); `alt_totals` → `betType:'over'|'under'`, `marketType:'totals'`, label "Alt Total". Player props stay DISPLAY-ONLY (see props rule) — do not make them pickable here.
3. Before deploying ANY sportsbook market addition/change, run a live click test (`_alt_clicktest.cjs` pattern: Playwright → switch sport → open the period tab → click one button per market → assert `window.TMR.currentSelectedPick` populated with correct team/line/odds/marketType AND the `.sportsbook-ticket-preview-card` slip reflects it AND the `.selected` class lands on the clicked button). A market is not "done" until at least one button per market type passes this assertion on the live URL.
4. Single-selection invariant: clicking any price clears the prior `.selected` and marks only the new one.
...
## Trend Spotter extended multi-sport history dataset (May 28, 2026) — HARD RULE
The Trend Spotter "Last 20 / Last 50 / Season" ranges pull from static, verified history files shipped with the site, NOT from the live trendspotter artifact (which only carries a 10-game window per matchup).
- **Files:** `static/data/{mlb,nba,nfl,nhl}/team-history.json`. One file per sport, keyed by full team name. Generated from `C:\Users\Nima\handicapping_tool\trends.db` `{mlb,nba,nfl,nhl}_games` tables.
- **Coverage:** MLB season>=2025 (~6,080 rows, 30 teams); NBA season>=2024 (~5,198 rows, 30 teams); NFL season>=2023 (~1,710 rows, 32 teams); NHL season>=2024 (~5,704 rows, 33 teams). Up to 250 most-recent games per team.
- **Fields per row:** `d`(date) / `opp` / `h`(is_home 0/1) / `s`(team_score) / `os`(opp_score) / `tl`(total_line) / `sp`(spread) / `ats` / `w` / `ml` / `te`(is_total_estimated) / `se`(is_spread_estimated). Rows flagged `te=1` or `se=1` are excluded from total/spread output (no estimated lines mixed into verified trends).
- **Frontend wiring:** `loadSportHistory(sport)` lazy-fetches per sport when that sport is selected; `extendedTrendForQuery()` builds a synthetic trend with full-history `source_rows` when state.range is in `EXTENDED_RANGES` and state.sport is in `EXTENDED_SPORTS` (MLB/NBA/NFL/NHL). Team aliases per sport in `TEAM_ALIASES` (e.g. "Athletics" -> "Oakland Athletics", "Utah Mammoth" -> "Utah Hockey Club").
- **Trend kinds unlocked by extended history:** `favorite` / `underdog` (moneyline sign on team's row), `favorite_ats` / `underdog_ats` (same), `after_win` / `after_loss` (next-row chronological lookup), `head_to_head` / `head_to_head_ats` / `head_to_head_over_under` (filter rows where opponent matches the current matchup's opponent). These stay disabled in source_window mode because the artifact doesn't expose role/sequence/H2H data.
- **Refresh contract:** regenerate the JSONs whenever the per-sport `_games` tables update. Bump the loader's `?v=` query string in `trendspotter.js` so CDN serves the new file. Source SQL pattern: `SELECT date, opponent, is_home, score, opponent_score, total_line, spread, ats_result, win, moneyline, is_total_estimated, is_spread_estimated FROM <sport>_games WHERE team=? AND season>=<min> ORDER BY date DESC LIMIT 250`.
- **No fake/estimated data.** Estimated-line rows are dropped from market_id=`total` and `spread` paths; `team_total` market stays artifact-only (no posted team totals in the extended history). NCAAB/NCAAF stay artifact-only - no historical games in `trends.db`.
...
- **Live renderer:** `sportsbook/index.html` → `renderRow(game, gameIndex, sport)` (the LOBBY board that paints into `#lobbyBoardRows .sportsbook-board-grid-body`, NOT the inline `_renderGamesList` `.game-row` path nor the `sportsbook-production-fix-persist-reliability.js` `.tmr-market-card` path — those are dead/secondary for the visible board). Gate with `lobbyIsMlb(game, sport)`; build the line with `lobbyPitcherHtml(game, side, sport)` rendered inside a `<span class="sb-team-stack">` wrapper so the name + pitcher stack vertically beside the logo.
- **Data:** backend Action Network MLB games carry NO pitcher field, so names are enriched from ESPN's public scoreboard (`site.api.espn.com/apis/site/v2/sports/baseball/mlb/scoreboard`, `competitors[].probables[].athlete.displayName`) via `lobbyEnsureMlbPitchers()` — fetched once, cached 10 min, matched by team displayName/nickname/abbr. Fail-open: on miss or error show `TBD`, never a broken blank, and never block odds rendering. `paintBoard` calls the enricher for MLB then paints; `_fullBoardCache` key is `v4mlbpitcher` (bump on any pitcher-markup change).
- **Styling:** `.sb-team-pitcher` (muted slate `#8ea0bc`, 11.5px/600, mobile 11px) + `.sb-team-pitcher-label` (`#6f7f99`, 700). Do not add a column, a second logo, or clutter the row — DraftKings-clean.
- **Must not break:** main MLB lines, alt team totals, bet-slip selection, submit/lock, existing styling, or other sports tabs. The pitcher line is appended inside the existing `.team-cell` only; odds cells/`selectGameBet` wiring are untouched.
- Future MLB sportsbook UI work must preserve this line. Verified live 2026-05-30 (desktop 1440 + mobile 390): 14 games × 2 real pitchers, NBA = 0 pitcher lines, TBD fallback confirmed.
