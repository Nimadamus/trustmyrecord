...
## Trend Spotter extended MLB history dataset (May 28, 2026) — HARD RULE
The Trend Spotter "Last 20 / Last 50 / Season" ranges pull from a static, verified history file shipped with the site, NOT from the live trendspotter artifact (which only carries a 10-game window per matchup).
- **File:** `static/data/mlb/team-history.json` - 30 teams keyed by full team name (e.g. `Los Angeles Angels`), last 250 games per team from season>=2025, generated from `C:\Users\Nima\handicapping_tool\trends.db` `mlb_games` table.
...
- **Frontend wiring:** `loadMlbHistory()` lazy-fetches the file when sport=MLB is selected; `extendedTrendForQuery()` builds a synthetic trend with full-history `source_rows` when state.range is in `EXTENDED_RANGES`. Team aliasing handled by `TEAM_ALIASES_MLB` (currently maps "Athletics" -> "Oakland Athletics").
- **Refresh contract:** regenerate the JSON whenever `mlb_games` is updated. Bump the loader's `?v=` query string in `trendspotter.js` so CDN serves the new file. Source SQL: `SELECT date, opponent, is_home, score, opponent_score, total_line, spread, ats_result, win, moneyline, is_total_estimated, is_spread_estimated FROM mlb_games WHERE team=? AND season>=2025 ORDER BY date DESC LIMIT 250`.
- **No fake/estimated data.** Estimated-line rows are dropped from market_id=`total` and `spread` paths; `team_total` market stays artifact-only (no posted team totals in the extended history).
