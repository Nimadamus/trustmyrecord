...
- **One features section only.** The homepage has exactly ONE platform-features grid: `.tmr-platform-section` ("More Than A Pick Tracker") inside `<main class="tmr-premium-home">`. The duplicate lower grid `.tmr-features` (id `what-it-is`, the all-caps "MORE THAN A PICK TRACKER") was removed May 24, 2026 (marker `DUPLICATE_FEATURES_SECTION_REMOVED_20260524`). Do not reintroduce a second features grid.
