...
## Gaming League Signup + Public Roster Pattern (May 25, 2026) — STANDARD
Reusable for any gaming-league acquisition page that collects open signups and shows a public roster (reference impl: MLB The Show Stat League, frontend `mlb-the-show-stat-league/index.html`, backend `routes/gaming.js`).
- **Storage = existing Render Postgres via the `/api/gaming` router**, NOT localStorage and NOT a new microservice. Add a `*_signups` table with the established self-creating pattern (`CREATE TABLE IF NOT EXISTS ... ` inside an `ensure<Table>()` helper called at the top of each handler) so no separate migration deploy is needed.
- **Endpoints** (mounted under `/api/gaming`): `GET /<league>/signups` returns public fields only (`id, display_name, platform, gamer_tag, note, created_at`); `POST /<league>/signup` is open (`optionalAuth`) + per-IP rate-limited (6/60s) + dedupes on `lower(display_name)+lower(gamer_tag)`; `DELETE /<league>/signups/:id` is `ADMIN_TOKEN`-gated for moderation/test cleanup.
- **Never collect or expose email / sensitive data** on the public roster. Only display name, platform, gamer tag, optional note.
- **Frontend**: vanilla `fetch` to `https://trustmyrecord-api.onrender.com`, escape all rendered values, show the empty state `No players signed up yet. Be the first to join.` when the list is empty, and re-fetch the roster after a successful POST.
- **Test cleanup is mandatory**: any test signup submitted during verification must be deleted before closing (no fake users on a public list, per Core Rule #4/#5). The pasted `ADMIN_TOKEN` in memory is WRONG — delete test rows by dispatching a one-shot GH Actions workflow on `nimadamus/trustmyrecord-backend` that calls the DELETE route with `secrets.TMR_ADMIN_TOKEN`, then remove the workflow file.
- Deploy backend `routes/gaming.js` as a single-file Contents-API PUT against the live remote `master` sha (do not push the stale local clone). Render autodeploys; poll the endpoint until the new shape appears.
- **Reuse the existing gaming engine for standings/stats** - do NOT build a parallel match/box-score/leaderboard system. Real match + box-score submission already lives in `routes/gaming.js` (`POST /matches`, `gaming_mlb_box_scores`, confirm/dispute) via the `/online-gaming/` flow, and standings come from `GET /api/gaming/leaderboard?game=<slug>&sort=wins`. League pages should wire their leaderboard to that endpoint (real data + real empty-state, `--` for stats not yet aggregated) and link result logging to `/online-gaming/`. Never fabricate players/stats to fill a leaderboard.
...
- **Never replace a functional tool's link with an SEO pillar link.** An info/pillar page (e.g. `/mlb-season-simulator/`) is an ADDITIONAL card; the interactive tool's link (e.g. `/mlb-simulator/`) must stay intact and distinct. Tools-hub cards must make the distinction clear (interactive tool vs info/guide page).
