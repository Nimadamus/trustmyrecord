...
function page(sport, mode, inputs, result, shell) {
  ...
  const fc = inputs.model && inputs.model.forecast && !inputs.model.forecast.error ? inputs.model.forecast : null;
  const howNba = fc ? `<p>Every game on the ${season} schedule is played out ${count(result.runs)} times. A game that is already final keeps its real result. Every other game is decided by NBA Forecasting V1, the frozen model behind these pages: an Elo rating replayed game by game, the opponent adjusted scoring margin, rest and back to backs. A scheduled game carries the forecast for its own date, so a club on the second night of a back to back is rated that way.</p>
<p>That model is not the game simulator. The simulator plays possessions and produces a box score, and on an untouched 2025-26 season it scored a Brier of 0.24021 at picking winners. The forecast layer scored 0.20603 over the same 1,231 games, walk forward, having been frozen before that season started. Lower is better, and 0.25 is a coin flip. So the odds here come from the forecast and the <a href="${L.hub}">NBA Simulator</a> keeps the job it is better at.</p>` : `<p>Every game on the ${season} schedule is played out ${count(result.runs)} times. A game that is already final keeps its real result. Every other game is decided by the calibrated win probability the <a href="${L.hub}">NBA Simulator</a> reports for that matchup: team strength solved from three seasons of results against the schedule each club actually played, a home court edge, and a calibration fitted on seasons the model never trained on.</p>
...
<main class="wrap lsim-wrap" id="lsim" data-sport="${sport}" data-mode="${mode}">
  ...
  <section class="lsim-copy">
    ...
    ${nba && fc
      ? `<p>The model that sets these probabilities was frozen before the 2025-26 season and then scored once on it: Brier 0.20603, log loss 0.59947, and the winner called correctly in 68.97% of 1,231 games. The game simulator scored 0.24021 on the same season and a coin flip scores 0.25000. No number here was tuned on that season, which is what makes it a test rather than a fit.</p>`
      : nba
      ? `<p>The game simulator's calibrated probability scored a Brier of 0.24021 on an untouched 2025-26 season, against 0.25000 for a coin flip.</p>`
      : `<p>Hockey is the hardest of the four to forecast, and this page says so with numbers rather than adjectives. The engine's calibrated probability scored a Brier of 0.24727 on an untouched 2025-26 season, against 0.25000 for a coin flip. Two replacements were measured on that same season on 2026-09-15, a recalibrated Elo with rest and back to backs (0.24940) and the frozen V2 anchor (0.24856), and both were worse, so nothing was changed. The engine keeps the job until something beats it.</p>`}
    <h2>Tiebreakers</h2>
    ${tiebreak}
