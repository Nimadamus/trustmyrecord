...
Header definitions and row render mapping must be treated as one locked unit. Future edits to the handicappers page must never separate table headers from row cell order; if a column is added, removed, renamed, or reordered, the header labels, CSS grid columns, static fallback rows, and dynamic row renderer must be changed together in the same review.
